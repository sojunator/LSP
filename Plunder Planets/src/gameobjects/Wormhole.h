...
public:
	...
	void StartLevel()
	{
		...
		if (m_spawned && !m_ship && m_time > 7.0f)
		{
			...
			m_ship->GetComponent<component::RigidBodyComponent>()->applyCentralImpulse((*(btVector3*)&shipForward * 100 + *(btVector3*)&shipUp * 70) * m_ship->GetComponent<component::RigidBodyComponent>()->GetMass());
		}
