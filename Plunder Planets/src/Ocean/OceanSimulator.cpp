...
namespace ocean
{
	...
	float Gauss()
	{
		...
		if (u1 < 1e-6f)
			u1 = 1e-6f;
		return sqrtf(-2 * logf(u1)) * cosf(2 * thomas::math::PI * u2);
	}

	// Phillips Spectrum
	// K: normalized wave vector, W: wind direction, v: wind velocity, a: amplitude constant
	float Phillips(thomas::math::Vector2 K, thomas::math::Vector2 W, float v, float a, float dir_depend)
	{
	...
	OceanSimulator::OceanSimulator(OceanParameter& params, ID3D11Device* pd3dDevice)
	{
		...
		thomas::math::Vector2* h0_data = new thomas::math::Vector2[height_map_size * sizeof(thomas::math::Vector2)];
		...
		pBlobUpdateSpectrumCS = thomas::graphics::Shader::Compile("../res/thomasShaders/ocean_simulator_cs.hlsl", "cs_4_0", "UpdateSpectrumCS");
		...
		pBlobQuadVS = thomas::graphics::Shader::Compile("../res/thomasShaders/ocean_simulator_vs_ps.hlsl", "vs_4_0", "QuadVS");
		pBlobUpdateDisplacementPS = thomas::graphics::Shader::Compile("../res/thomasShaders/ocean_simulator_vs_ps.hlsl", "ps_4_0", "UpdateDisplacementPS");
		pBlobGenGradientFoldingPS = thomas::graphics::Shader::Compile("../res/thomasShaders/ocean_simulator_vs_ps.hlsl", "ps_4_0", "GenGradientFoldingPS");
		assert(pBlobQuadVS);
		...
		vb_desc.ByteWidth = 4 * sizeof(thomas::math::Vector4);
		...
		ID3D10Blob* CSBlob;
		CSBlob = thomas::graphics::Shader::Compile("../res/shaders/OceanCollisionCS.hlsl", "cs_4_0", "CSMain");
		assert(CSBlob);
	...
	OceanSimulator::~OceanSimulator()
	{
		...
		SAFE_RELEASE(m_oceanColliderCS);
		SAFE_RELEASE(m_oceanCollisionDataBuffer);
		SAFE_RELEASE(m_oceanCollisionHeightDataBuffer);
		SAFE_RELEASE(m_oceanCollisionHeightDataUAV);
	...
	void OceanSimulator::initHeightMap(OceanParameter& params, thomas::math::Vector2* out_h0, float* out_omega)
	{
		int i, j;
		thomas::math::Vector2 K, Kn;

		thomas::math::Vector2 wind_dir = params.wind_dir;
		wind_dir.Normalize();
		...
		for (i = 0; i <= height_map_dim; i++)
		{
			...
			K.y = (-height_map_dim / 2.0f + i) * (2 * thomas::math::PI / patch_length);

			for (j = 0; j <= height_map_dim; j++)
			{
				K.x = (-height_map_dim / 2.0f + j) * (2 * thomas::math::PI / patch_length);
	...
	void OceanSimulator::updateDisplacementMap(float time)
	{
		...
		UINT strides[1] = { sizeof(thomas::math::Vector4) };
		UINT offsets[1] = { 0 };
	...
	void OceanSimulator::UpdateOceanCollision()
	{
		...
		ID3D11Buffer* nullCBuffer = { NULL };
		ID3D11UnorderedAccessView* nullUAV = { NULL };
		thomas::ThomasCore::GetDeviceContext()->CSSetShader(nullptr, NULL, 0);
		thomas::ThomasCore::GetDeviceContext()->CSSetConstantBuffers(0, 1, &nullCBuffer);
	...
	{
		return m_oceanCollisionHeightData[index];
	}
